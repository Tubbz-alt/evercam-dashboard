...
getArchivesHtml = (archives) ->
  ...
  archive_from_date = getDates(archives.from_date, archives.created_at)
  archive_to_date = getDates(archives.to_date, archives.created_at)
  ...
  if archives.type is "url"
    ...
    html += "          <span class='spn-label' style='margin-top:3px;'><i class='fas fa-calendar-alt font-16' style='color:#aaaaaa;'></i></span><div class='div-snapmail-values snapmail-title #{hide_dates}' style='margin-top:3px;'><span class='small-text'>&nbsp;&nbsp;#{archive_from_date} - #{archive_to_date}</span><span class='line-end'></span></div><div class='clear-f'></div>"
  html += "    </div></div>"
  html += "    </div>"
  html += "</div>"
  html
    ...
    if type is "url"
      ...
      return "<a class='archive-actions dropdown-toggle archive-title' href='#'' title='share' data-id='#{id}' data-type='#{type}' data-toggle='modal' data-target='#modal-archive-info'><i class='fas fa-share-alt'></i> Share</a>" +
        ...
        "<ul class='dropdown-menu'>
          <li><a class='download-animation archive-icon archive-download' href='javascript:;' data-camera-id='#{camera_id}' data-type='#{type}' data-from='#{from_dt}' data-to='#{to_dt}' data-download-target='#gif-#{id}' title='Download GIF (Good for emails)'><i class='fa fa-download'></i> GIF</a></li>" +
          "<li><a class='download-animation archive-icon archive-download' href='javascript:;' data-camera-id='#{camera_id}' data-type='#{type}' data-from='#{from_dt}' data-to='#{to_dt}' data-download-target='#mp4-#{id}' title='Download MP4 (Good for everything else)'><i class='fa fa-download'></i> MP4</a></li></ul>" +
        "</div>" + div.html()
    else
      mp4_media_url = "#{Evercam.API_URL}cameras/#{camera_id}/archives/#{id}.mp4"
      is_enable = ""
      ...
      publicButtons = renderIsPublicPlayer(camera_id ,id, type, status, media_ispublic, true)

      return "<div class='dropdown'>" + share_button +
        "<div style='display:inline-block;cursor:pointer;' class='archive-actions'><a class='download-animation archive-icon archive-download' data-camera-id='#{camera_id}' data-type='#{type}' data-from='#{from_dt}' data-to='#{to_dt}' data-download-target='#mp4clip-#{id}' title='Download MP4'><i class='fa fa-download'></i> Download</a></div>" +
...
renderbuttons = (row, type, set, meta) ->
  ...
  if Evercam.Camera.has_edit_right || row.requested_by is Evercam.User.username
    ...
    inputDelete.attr("camera_id", row.camera_id)
...
rendersharebuttons = (row, type, set, meta) ->
  ...
  if row.status is "Completed"
    if row.type is "url"
      ...
      download_link = "<div class='float-left'><a class='archive-actions download-animation archive-icon archive-download' data-camera-id='#{row.camera_id}' data-from='#{row.from_date}' data-to='#{row.to_date}' data-type='#{row.type}' href='javascript:;'' data-download-target='#mp4clip-#{row.id}'><i class='fa fa-download' title='Download'></i></a></div>"
      ...
      if row.type is "file" || row.type is "edit"
        ...
        download_link = '<div class="dropdown"><a class="archive-actions dropdown-toggle" href="#" data-toggle="dropdown" title="Download"><i class="fa fa-download"></i></a>' +
                        "<ul class='dropdown-menu'><li><a class='download-animation archive-icon archive-download' data-camera-id='#{row.camera_id}' data-type='compare' data-from='#{row.from_date}' data-to='#{row.to_date}' href='javascript:;' data-download-target='#gif-#{row.id}' title='Download GIF (Good for emails)'><i class='fa fa-download'></i> GIF</li></a>" +
                          "<li><a class='download-animation archive-icon archive-download' href='javascript:;' data-type='compare' data-camera-id='#{row.camera_id}' data-from='#{row.from_date}' data-to='#{row.to_date}' data-download-target='#mp4-#{row.id}' title='Download MP4 (Good for everything else)'><i class='fa fa-download'></i> MP4</a></li></ul>" +
                        '</div>'
...
onClickPublicInputToggle = ->
  $("#archives-tab").off('click', '.toggle_input_public').on 'click', '.toggle_input_public', (e) ->
    ...
    cameras_id = is_checked.attr('camera-id')
    ...
    if is_checked.is(":checked")
      ...
      $("#popup-warning-modal").off('click', '#make-archive-public').on 'click', '#make-archive-public', ->
        ...
        makePublic(is_checked, cameras_id, id, true)
    else
      flag = false
      ...
      makePublic(is_checked, cameras_id, id, flag)

makePublic = (is_checked, camera_id, id, flag) ->
  ...
  if typeArchive isnt "compare"
    settings =
      ...
      success: onSuccess
      type: 'PATCH'
      url: "#{Evercam.API_URL}cameras/#{camera_id}/archives/#{id}?api_id=#{Evercam.User.api_id}&api_key=#{Evercam.User.api_key}"
...
getTitle = (row, type, set, meta) ->
  start_index = row.embed_code.indexOf("#{row.camera_id}")
  ...
  row_from_date = getDates(row.from_date, row.created_at)
  row_to_date = getDates(row.to_date, row.created_at)
  ...
  if row.type is "url"
    ...
    if row.type is "compare"
      mp4Url = "#{Evercam.API_URL}cameras/#{row.camera_id}/compares/#{row.id}.mp4"
      fa_class = "<svg fill='#ffffff' height='18' viewBox='0 0 24 24' width='18'>
        <path d='M0 0h24v24H0z' fill='none'/>
        <path d='M10 3H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h5v2h2V1h-2v2zm0 15H5l5-6v6zm9-15h-5v2h5v13l-5-6v9h5c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2z'/>
        </svg>"
    ...
    return "<div class='gravatar-placeholder'><img class='gravatar' src='#{row.thumbnail_url}'><div class='type-icon-alignment'>#{fa_class}</div><div class='float-left'></div>
      ...
      <br /><small class='blue'>From #{row_from_date} to #{row_to_date}</small></div></div>
...
renderIsPublic = (row, type, set, meta) ->
  renderIsPublicPlayer(row.camera_id, row.id, row.type, row.status, "#{row.public}", false)

renderIsPublicPlayer = (cameras_id, id, type, status, media_ispublic, add_label) ->
  if status is "Completed"
    if type is "url" || type is "compare"
      ...
      return "<div id='siderbar'>
                <label class='label toggle title'>
                  #{radio_label}
                  <input type='checkbox' class='toggle_input_public toggle_input_public_clip' alt='#{id}' camera-id='#{cameras_id}' archive_type='#{type}' #{enabled} />
...
getDates = (times, created_at) ->
  offset = moment.parseZone(created_at).utcOffset() * 60
...
playClip = ->
  ...
  $("#archives").on "click", ".download-animation", ->
    ...
    camera_name = $("#txt-camera-id").val().replace(/ /g, "-")
    if type is "edit"
...
deleteClip = ->
  $('#archives').on 'click','.delete-archive2', ->
    ...
    api_url = "#{Evercam.API_URL}cameras/#{control.attr("camera_id")}/archives/#{control.attr("archive_id")}?api_id=#{Evercam.User.api_id}&api_key=#{Evercam.User.api_key}"
    if control.attr("archive_type") is "compare"
...
modal_events = ->
  $("#archives").on "click", ".archive-title-color", ->
    ...
    newTime = moment(media_time)
    media_from_date = getDates(media_from, media_time)
    media_to_date = getDates(media_to, media_time)
    $("#txt-media-url").val(media_url)
    ...
    $('#txt_title').text(media_title)
    $('#archive-autor').html("Created on #{moment(newTime).format('MMMM Do YYYY, H:mm:ss')} by #{media_autor}</br>")
    $('#archive-dates').html("From #{media_from_date} to #{media_to_date}")
    $("#txt-archive-type").val(type)
    $("#txt-archive-id").val(id)
    $("#txt-camera-id").val(camera_id)
    $("#txt-archive-title").val(media_title)
...
update_request_url = (id) ->
  if id
    url = "#{Evercam.request.rootpath}/#{id}"
  else
    url = "#{Evercam.request.rootpath}"
  ...
  $("#text-info").off('click', '#edit-archive-title').on 'click', '#edit-archive-title', (e) ->
    ...
    $("#text-info").off('click', '#update-archive').on 'click', '#update-archive', (e) ->
      ...
      settings =
        ...
        success: onSuccess
        type: 'PATCH'
        url: "#{Evercam.API_URL}cameras/#{camera_id}/#{controller}/#{id}?api_id=#{Evercam.User.api_id}&api_key=#{Evercam.User.api_key}"
      sendAJAXRequest(settings)
